...
public class PhysicsTest : MonoBehaviour
{
    ...
    public float radius = 1.31f, mass = 1f, omega = 0f, fmax = 0f, MOI, pi = Mathf.PI, theta = 0f, torque = 0f,maxspeed = 50f;
    ...
    private void FixedUpdate()
    {
        ...
        else
        {
            ...
            appliedForce=new Vector2(0f, 0f);
        }
        ...
        Square.localPosition = Wheel.localPosition + new Vector3(.55f, 2.6f, 0f);
        previousHits = hits;
    }
    ...
    {
        if (Input.GetKey("d"))
            appliedForce = new Vector2(12f*mass, 0f);
        else if (Input.GetKey("a"))
            appliedForce = new Vector2(-12f*mass, 0f);
        else if (Input.GetKey("w"))
            appliedForce = new Vector2(0f, 20f*mass);
        else
       ...
       float radius,float mu, Vector2 normal)
   {
    ...
    private void ApplyForces(Vector2 netForce, float dt)
    {
        ...
        omega = Math.Min(omega, maxspeed/1.3f);
        // Update rotation
        theta += omega * dt;
