...
public class Front_wheel : MonoBehaviour
{
    ...
    [SerializeField] public Vector2 tangent = Vector2.zero, normal = Vector2.zero,netForce = Vector2.zero, friction = Vector2.zero;
    ...
    private void FixedUpdate()
    {
        ...
        MainCam.localPosition=(rearWheel.localPosition+Wheel.localPosition)/2f + new Vector3(0f, 0f, -10f);
    ...
    private void MaintainOscillation(float dt)
    {
        ...
        if (Math.Abs(currentDistance-7.2f)>0.01f)
        {
            ...
            Vector2 force = totalForceMagnitude * direction;
            Debug.Log("force : " + force);

            // Apply equal and opposite forces to each wheel
            ApplyForces(force, dt); // Front wheel force application
            Script_R.ApplyForces(-force, dt); // Rear wheel force application
        }
