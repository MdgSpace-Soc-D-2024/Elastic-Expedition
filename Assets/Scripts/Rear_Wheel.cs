...
public class Rear_wheel : MonoBehaviour
{
    ...
    [SerializeField] public Vector2 tangent = Vector2.zero, normal = Vector2.zero, appliedForce = Vector2.zero,netForce = Vector2.zero, friction = Vector2.zero,Netnormal=Vector2.zero;

    public RaycastHit2D[] hits;
    private RaycastHit2D[] previousHits = new RaycastHit2D[0];

    private ForceController forceController2;
    private void Start()
    {
        pos = Wheel.position;
        MOI = 0.5f * mass * radius * radius; // Moment of inertia for a solid disk

        forceController2 = GetComponent<ForceController>();
    }
    ...
    private void FixedUpdate()
    {
        ...
        appliedForce = forceController2 != null ? forceController2.AppliedForce : Vector2.zero;
    ...
    private void HandleCollision(RaycastHit2D[] hits, float dt, bool flag)
    {
        ...
        if (Math.Abs(velocity.x) > 0.3f)
            HandleFrictionNormally(ref velocity, ref omega, hits[0].point, Netnormal,normal);
    }
    private void HandleFrictionNormally(ref Vector2 velocity1, ref float omega1, Vector2 contactPoint, Vector2 collisionNormal,Vector2 normal)
    {
